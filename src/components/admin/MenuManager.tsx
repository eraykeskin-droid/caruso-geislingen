...
const MenuManager = () => {
    ...
    const [isSubcategoryCategoryDropdownOpen, setIsSubcategoryCategoryDropdownOpen] = useState(false);
    ...
    const deleteCategory = (id: string) => {
        setConfirmModal({
            ...
            message: 'Möchten Sie diese Kategorie wirklich unwiderruflich löschen?',
            onConfirm: () => {
    ...
    const deleteSubcategory = (parentId: string, subId: string) => {
        setConfirmModal({
            title: 'Unterkategorie löschen',
            message: 'Möchten Sie diese Unterkategorie wirklich unwiderruflich löschen?',
            onConfirm: () => {
                const newCategories = categories.map(c =>
    ...
    const deleteItem = (parentId: string, itemId: string, grandParentId?: string) => {
        setConfirmModal({
            title: 'Artikel löschen',
            message: 'Möchten Sie diesen Artikel wirklich unwiderruflich löschen?',
            onConfirm: () => {
                const catId = grandParentId || parentId;
                const newCategories = categories.map(c => {
                    if (c.id !== catId) return c;
        ...
        <div className="relative">
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/80">
                    <div className="w-full max-w-md bg-[#0a0a0a] border border-white/10 rounded-lg p-8 shadow-2xl">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={() => setModal(null)} className="text-gray-500 hover:text-white transition-colors cursor-pointer">
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="space-y-4">
                                ...
                                <>
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <input
                                            ...
                                            className="bg-black/50 border border-white/10 rounded-none px-4 py-3 focus:outline-none focus:border-secondary transition-colors text-white text-sm w-full h-[46px]"
                                        />
                                    </div>
                                ...
                                <>
                                    ...
                                    {modal.mode === 'add' && (
                                        <div className="flex flex-col gap-2">
                                            ...
                                            <div className="relative">
                                                <div
                                                    className="bg-black/50 border border-white/10 rounded-none px-4 py-3 text-sm cursor-pointer hover:border-secondary/50 transition-colors flex items-center justify-between h-[46px]"
                                                    onClick={() => setIsSubcategoryCategoryDropdownOpen(!isSubcategoryCategoryDropdownOpen)}
                                                >
                                                    <span className="text-white truncate flex-1 mr-2">
                                                        {categories.find(c => c.id === modal.parentId)?.name || 'Kategorie wählen...'}
                                                    </span>
                                                    <ChevronDown size={14} className={`text-secondary shrink-0 transition-transform ${isSubcategoryCategoryDropdownOpen ? 'rotate-180' : ''}`} />
                                                </div>
                                                {isSubcategoryCategoryDropdownOpen && (
                                                    <div className="absolute z-[250] left-0 right-0 mt-1 bg-black border border-white/10 max-h-52 overflow-y-auto w-full shadow-2xl">
                                                        {categories.map((cat) => (
                                                            <div
                                                                ...
                                                                onClick={() => {
                                                                    setModal({ ...modal, parentId: cat.id });
                                                                    setIsSubcategoryCategoryDropdownOpen(false);
                                                                }}
                                                            >
                                                                {cat.name}
                                                            </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="flex flex-col gap-2">
                                                ...
                                                <div className="relative">
                                                    ...
                                                    >
                                                        ...
                                                        <ChevronDown size={14} className={`text-secondary shrink-0 transition-transform ${isCategoryDropdownOpen ? 'rotate-180' : ''}`} />
                                                    </div>
                                            ...
                                            <div className="flex flex-col gap-2">
                                                ...
                                                <div className="relative">
                                                    ...
                                                    >
                                                        <span className={`${modal.data.subcategory_id ? "text-white" : "text-white/25"} truncate flex-1 mr-2`}>
                                                            {modal.data.subcategory_id
                                                                ? modal.subcategories?.find((s: SubCategory) => s.id === modal.data.subcategory_id)?.name
                                                                : 'Keine'}
                                                        </span>
                                                        <ChevronDown size={14} className={`text-secondary shrink-0 transition-transform ${isSubcategoryDropdownOpen ? 'rotate-180' : ''}`} />
                                                    </div>
                                            ...
                                            <div className="grid grid-cols-1 gap-2">
                                                {showSubs && (
                                                    <button
                                                        ...
                                                        className="flex items-center justify-center gap-2 px-4 py-2.5 bg-white/5 border border-white/10 text-gray-300 hover:text-white hover:bg-white/10 transition-all text-[10px] font-bold uppercase tracking-widest cursor-pointer"
                                                    >
                            ...
                            <div className="flex gap-3 mt-6">
                                        ...
                                        <button
                                            onClick={() => {
                                                ...
                                                setConfirmModal({
                                                    ...
                                                    message: 'Möchten Sie dieses Element wirklich unwiderruflich löschen?',
                                                    onConfirm: onDelete
                                                });
                                            }}
                                            className="w-12 py-3 bg-red-500/10 text-red-500 border border-red-500/20 font-bold uppercase tracking-widest rounded hover:bg-red-500 hover:text-white transition-all active:scale-95 flex items-center justify-center cursor-pointer"
                                            title="Löschen"
                                        >
                                            <Trash2 size={16} />
                                        </button>
                                            ...
                                            <button
                                                onClick={() => {
                                                    setModal({
                                                        ...modal,
                                                        mode: 'add',
                                                        parentId: modal.grandParentId,
                                                        grandParentId: modal.grandParentId,
                                                        data: {
                                                            ...modal.data,
                                                            id: undefined,
                                                            name: `${modal.data.name} (Kopie)`
                                                        }
                                                    });
                                                }}
                                                className="w-12 py-3 bg-white/5 text-gray-400 border border-white/10 font-bold uppercase tracking-widest rounded hover:bg-white/10 hover:text-white transition-all active:scale-95 flex items-center justify-center cursor-pointer"
                                ...
                                <button
                                    onClick={handleSave}
                                    className={`${modal.mode === 'edit' ? 'flex-[2]' : 'w-full'} text-xs py-3 bg-secondary text-black font-bold uppercase tracking-widest rounded hover:bg-white transition-all active:scale-95 flex items-center justify-center gap-2 cursor-pointer`}
                                >
            ...
            <div className="fixed bottom-8 right-8 z-[150]">
                    ...
                    <div className="absolute bottom-full right-0 mb-4 bg-[#111] border border-white/10 shadow-2xl flex flex-col min-w-[200px] overflow-hidden">
                        {categories.length > 0 && (
                            <button
                                onClick={() => {
                                    ...
                                    setModal({ type: 'item', mode: 'add', data: { name: '', price: 0, unit: '', info: '', allergens: '', subcategory_id: '' }, parentId: firstCat?.id, grandParentId: firstCat?.id, subcategories: firstCat?.subcategories || [] });
                                    setShowAddMenu(false);
                                }}
                                className="flex items-center gap-3 px-6 py-4 text-[11px] font-bold uppercase tracking-widest text-gray-300 hover:text-black hover:bg-secondary transition-all border-b border-white/5 text-left cursor-pointer"
                            >
                        ...
                        {categories.length > 0 && (
                            <button
                                ...
                                className="flex items-center gap-3 px-6 py-4 text-[11px] font-bold uppercase tracking-widest text-gray-300 hover:text-black hover:bg-secondary transition-all text-left cursor-pointer"
                            >
                ...
                <button
                    ...
                    className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all active:scale-90 border border-white/10 cursor-pointer ${showAddMenu ? 'bg-secondary text-black' : 'bg-[#111] text-secondary hover:bg-secondary hover:text-black'}`}
                >
                    <Plus size={28} />
                </button>
            </div>
                ...
                <div className="flex-grow flex flex-col gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 min-w-0">
                        ...
                        >
                            ...
                            {!!category.is_special && (
                                <span className="inline-flex text-[8px] sm:text-[8.5px] text-black bg-secondary px-1 sm:px-1.5 py-0 rounded-none uppercase tracking-wider font-semibold flex-shrink-0">
                                    {category.badge_text || 'Spezial'}
                                </span>
                            )}
                        </div>

                        {/* Counts, Add Item, Menu. Wrap on mobile */}
                        <div className="flex items-center justify-start sm:justify-end gap-1.5 sm:gap-2 w-auto h-[28px] sm:h-[32px] shrink-0">
                            ...
                            <div className="flex items-center gap-2 h-full">
                                <button onClick={onToggleVisibility} className="w-9 sm:w-10 h-full flex items-center justify-center bg-white/5 border border-white/10 text-gray-500 hover:text-secondary hover:border-secondary/30 transition-all cursor-pointer" title={category.is_hidden ? "Einblenden" : "Ausblenden"}>
...
const SubCategoryBox = ({ subcategory, categoryId, isCollapsed, onToggleCollapse, onEdit, onEditItem, onDeleteItem }: {
    ...
    return (
        <div className="bg-black/20 border border-white/5 rounded-none p-3 sm:p-4 group/sub hover:border-secondary/20 transition-all">
...
const SortableItem = ({ item, onEdit }: {
    ...
    return (
        <div
            onClick={onEdit}
            className="flex items-start gap-1 p-3 bg-black/40 rounded-none border border-white/5 group hover:border-secondary/20 transition-all cursor-pointer"
        >
            <div className="flex-grow flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0 min-w-0">
                <div className="flex flex-col min-w-0">
                    <h4 className="text-white font-bold text-xs truncate uppercase tracking-wide">{item.name}</h4>
                    <p className="text-[10px] text-gray-500 italic truncate mt-0.5">{item.info || 'Keine Info'}</p>
                </div>

                <div className="flex items-center justify-between sm:justify-end gap-3 sm:gap-4 shrink-0 h-7 w-full sm:w-auto sm:mt-0">
                    <span className="text-secondary font-bold text-xs sm:order-last sm:min-w-[50px] sm:text-right">
                        {typeof item.price === 'number' ? item.price.toFixed(2) : parseFloat(item.price as string).toFixed(2)}€
                    </span>
                    {item.unit && (
                        <div className="flex items-center justify-center text-[9px] text-gray-400 bg-white/5 px-2 py-0.5 rounded border border-white/10 shrink-0 font-bold uppercase tracking-widest w-fit h-6 sm:order-first">
                            {item.unit}
                        </div>
                    )}
                </div>
    ...
    return (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-6 bg-black/90 backdrop-blur-sm">
            <div className="w-full max-w-md bg-[#0a0a0a] border border-white/10 rounded-lg p-8 shadow-2xl flex flex-col max-h-[80vh]">
                ...
                <div className="flex-grow overflow-y-auto pr-2 custom-scrollbar mb-8 select-none">
                ...
                <div className="flex gap-3 mt-auto shrink-0 pt-6 border-t border-white/5">
                    <button
                        ...
                        className="w-full text-xs py-3 bg-secondary text-black font-bold uppercase tracking-widest rounded hover:bg-white transition-all active:scale-95 flex items-center justify-center gap-2 cursor-pointer"
                    >
                        <Save size={16} /> Speichern
                    </button>
                </div>
...
const SortableOrderItem = ({ id, name }: { id: string, name: string }) => {
    ...
    return (
        ...
        >
            <button {...attributes} {...listeners} className="text-gray-500 hover:text-white cursor-grab active:cursor-grabbing shrink-0 touch-none">
                <GripVertical size={18} />
            </button>
            <span className="text-sm text-white font-bold uppercase tracking-wide truncate select-none">{name}</span>
        </div>
    );
};
